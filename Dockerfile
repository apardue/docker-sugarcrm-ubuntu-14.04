...
RUN apt-get update &&\
    apt-get upgrade -y &&\
    DEBIAN_FRONTEND=noninteractive apt-get -y install git subversion curl apache2 php5 php5-cli libapache2-mod-php5 php5-mysql php-apc php5-imap php5-gd php5-curl php5-memcached php5-mcrypt php5-mongo php5-sqlite mysql-client &&\
    apt-get clean &&\
...
ADD files/vhost.conf /etc/apache2/sites-available/sugarcrm.conf
